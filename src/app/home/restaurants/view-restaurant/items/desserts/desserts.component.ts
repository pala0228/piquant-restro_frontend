...
export class DessertsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // get desserts by restaurant Id
    if (this.restaurantId) {
      this.getDesserts(this.restaurantId, ITEM_CATEGORY.DESSERTS);
    }
    ...
    this.websocketIoService.listenToServer(CONNECTION_TYPE.ITEM).subscribe((data) => {
      if (data.item.itemCategory === ITEM_CATEGORY.DESSERTS) {
        switch (data.action) {
    ...
    // update desserts quantity and total price as per cart items if exist
    this.cartItemsMap = this.cartService.getitems();
    let cartItems = [];
    if (this.cartItemsMap.size > 0) {
      for (let [key, value] of this.cartItemsMap) {
        if (value.itemCategory === ITEM_CATEGORY.DESSERTS) {
      ...
      if (cartItems.length > 0) {
        setTimeout(() => {
          cartItems.forEach(item => {
            let index = this.desserts.findIndex(object => object.id === item.id);
            if (index !== -1) {
              this.desserts[index].itemQuantity = item.itemQuantity;
              this.desserts[index].itemTotalPrice = item.itemTotalPrice;
            }
          });
        }, 1000);
      }
  ...
  onDeleteItem(itemId) {
    ...
    this.viewRestaurantService.deleteItem(itemId, this.restaurantId).subscribe((httpResponse: ItemCRUDResponse) => {
      if (httpResponse.status.code === 200) {
        this.desserts = this.desserts.filter(object => object.id !== itemId);
  ...
  onChangedItemCategory(itemId) {
    this.desserts = this.desserts.filter((item) => item.id !== itemId);
  }
  /**
   * @ignore
   * method to get desserts available from database.
   */
  getDesserts(restaurantId: string, itemCategory: string) {
    // Basic initializations required
    ...
    this.viewRestaurantService.getFoodItems(restaurantId, itemCategory).subscribe((httpResponse: FetchItemsResponse) => {
      if (httpResponse.status.code === 200) {
        httpResponse.response.items.forEach((item) => {
          ...
          this.desserts.push(updatedItem);
        });
  ...
  private addItem(item) {
    let updatedItem = this.getUpdatedItem(item);
    this.desserts = [...this.desserts, ...[updatedItem]];
  }
  /**
   * @ignore
   */
